  ...
  context "User is logged in as a service operator" do
    ...
    context "When the claimant has another approved claim in the same payroll window, with inconsistent personal details" do
      ...
      scenario "User is informed that the claim cannot be approved" do
        ...
        expect(page).to have_field("Approve", disabled: true)
        expect(page).to have_content("This claim cannot be approved, since we will not be able to pay it in the current payroll month. This is because claim #{approved_claim.reference} from the same claimant has already been approved in the current payroll month and it has different values for bank sort code and bank account number.")
      end
    end
