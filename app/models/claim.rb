...
class Claim < ApplicationRecord
  ...
  validates :payroll_gender, on: [:gender, :submit], presence: {message: "Choose the option for the gender your school’s payroll system associates with you"}, unless: :ecp_skip_validation

  validates :first_name, on: [:name, :submit], presence: {message: "Enter your first name"}, unless: :ecp_skip_validation
  ...
  validates :surname, on: [:name, :submit], presence: {message: "Enter your last name"}, unless: :ecp_skip_validation
  validates :surname, length: {maximum: 100, message: "Last name must be 100 characters or less"}

  validates :address_line_1, on: [:address, :submit], presence: {message: "Enter your building and street address"}, unless: :ecp_skip_validation
  ...
  validates :postcode, on: [:address, :submit], presence: {message: "Enter your postcode"}, unless: :ecp_skip_validation
  validates :postcode, length: {maximum: 11, message: "Postcode must be 11 characters or less"}

  validates :date_of_birth, on: [:"date-of-birth", :submit], presence: {message: "Enter your date of birth"}, unless: :ecp_skip_validation

  validates :teacher_reference_number, on: [:"teacher-reference-number", :submit], presence: {message: "Enter your teacher reference number"}, unless: :ecp_skip_validation
  validate :trn_must_be_seven_digits

  validates :national_insurance_number, on: [:"national-insurance-number", :submit], presence: {message: "Enter your National Insurance number"}, unless: :ecp_skip_validation
  validate :ni_number_is_correct_format

  validates :has_student_loan, on: [:"student-loan", :submit], inclusion: {in: [true, false], message: "Select yes if you have a student loan"}, unless: :ecp_skip_validation
  ...
  validates :student_loan_plan, on: [:submit], presence: {message: "We have not been able determined your student loan repayment plan. Answer all questions about your student loan."}, unless: :ecp_skip_validation
  ...
  validates :email_address, on: [:"email-address", :submit], presence: {message: "Enter an email address"}, unless: :ecp_skip_validation
  ...
  validates :banking_name, on: [:"bank-details", :submit], presence: {message: "Enter the name on your bank account"}, unless: :ecp_skip_validation
  validates :bank_sort_code, on: [:"bank-details", :submit], presence: {message: "Enter a sort code"}, unless: :ecp_skip_validation
  validates :bank_account_number, on: [:"bank-details", :submit], presence: {message: "Enter an account number"}, unless: :ecp_skip_validation

  validates :payroll_gender, on: [:"payroll-gender-task", :submit], presence: {message: "You must select a gender that will be passed to HMRC"}, unless: :ecp_skip_validation
